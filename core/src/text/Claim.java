...
public class Claim {
    ...
    public void send() throws Exception {
        ...
        int count = (rq_text.length() - rq_text.replace("(", "").length());
        int count2 = (rq_text.length() - rq_text.replace(")", "").length());


        if (!command.equals(Command.GENERIC_SAY) && (!command.equals(Command.SAY)))
            if (count != 1 || count2 != 1){
    ...
    public void removeQuoteText() {
        ...
        int count = this.text.length() - this.text.replace("\"", "").length();

        if (count != 2 && count != 0){
            console.log("Error, mismatch quotes with line " + this.text);
            this.rq_text = "";
            this.text = "\"ERROR\"";
            return;
        }
    ...
    private Command parseCommand() throws Exception {
        ...
        //printToLine();
        console.log("Bad Command Choice at :" + this.r_text);
        return Command.WAIT;
//        throw new Exception("Bad Command Choice");
    ...
    private void parseText() {
        ...
        while (m.find()) {
            if (this.command.equals(Command.GENERIC_SAY) && m.group().length() >= 2)
                dialog.add(m.group().substring(1,m.group().length()-1).trim());
            else
                dialog.add(m.group());
        }
    }
    ...
    public void parseParameters() {
        ...
        Pattern pattern = Pattern.compile("([A-Za-z_=0123456789]*)");
